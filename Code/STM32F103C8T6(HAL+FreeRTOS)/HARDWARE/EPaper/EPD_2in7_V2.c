...
uint8_t epd_main_updata(epd_dev_v2_t *dev, epd_screen_element_t elememt)
{
    ...
    case EPD_MAIN_SCREEN_ELEMENT_WiFi_ICON:
        // memset(dev->p_w25qxx_dev_st->read_buf, 0, EPD_MAIN_PAGE_ICON_SIZE);
        if (dev->p_w25qxx_dev_st->state_em == W25QXX_STATE_ONLINE) // 如果在线就读取flash中的数据否则使用默认图标
        {
            INFO_PRINT("read wifi icon from w25qxx\r\n");
            dev->p_w25qxx_dev_st->read_data_cb(dev->p_w25qxx_dev_st->read_buf, WIFI_UNLINKED_ICON_ADDER,
        ...
        {
            INFO_PRINT("read wifi icon from flash %d\r\n", dev->p_w25qxx_dev_st->state_em);
    ...
    case EPD_MAIN_SCREEN_ELEMENT_WEATHER_ICON:
        ...
        {
            INFO_PRINT("read wrather icon from flash %d\r\n", dev->p_w25qxx_dev_st->state_em);
            Paint_DrawBitMap_Paste(gImage_sunny_icon,
                                   dev->main_element_attr[EPD_MAIN_SCREEN_ELEMENT_WEATHER_ICON].x_start,
                                   dev->main_element_attr[EPD_MAIN_SCREEN_ELEMENT_WEATHER_ICON].y_start,
                                   48, 48, 1);
        }
 ...
 * @param {w25qxx_device_t} *p_w25qxx_dev   w25qxx设备, 需要从里面读取图标数据
